...
bool ChatServerInstance::loginRequest(ChatUser* pUser, CS_LoginPacket_Req* pPacket)
{
	...
	if (true == loginResult)
	{
		...
		if (false == isAlready)
		{
			//LOGTRACE("ALREADY LOGIN NOW (Member Id %d, User ID %s)", 
			//	frontInfo[0].getInt(), pPacket->requestId);
		}

		else
		{
			//LOGTRACE("LOGIN SUCCESS (Member Id %d, User ID %s, Nickname %s)", 
			//	frontInfo[0].getInt(), pPacket->requestId, frontInfo[3].getChar());
